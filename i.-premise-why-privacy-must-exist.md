...
While Solana offers speed and low transaction costs, it lacks native privacy. Every transaction is publicly visible, exposing user activity and compromising confidentiality. Voidify addresses this gap with two privacy modes: fixed-denomination Classic pools and the flexible-amount [Nova Privacy Pool](iii.-nova-privacy-pool.md). Both let users withdraw to another wallet without leaving a visible on-chain link to the source deposit.
...
Solana 提供了速度和低交易成本，但它没有原生隐私。每一笔交易都是公开可见的，这会暴露用户活动并损害机密性。Voidify 提供两种隐私模式来弥补这一缺口：固定面额的 Classic 池，以及灵活金额的 [Nova 隐私池](iii.-nova-privacy-pool.md)。两种模式都允许用户把资金提取到另一个钱包，而不留下与来源存款之间的可见链上链接。
...
Solana обеспечивает высокую скорость и низкие транзакционные издержки, но не имеет встроенной приватности. Каждая транзакция публично видима, раскрывает активность пользователя и подрывает конфиденциальность. Voidify решает эту проблему двумя режимами: Classic pools с фиксированными номиналами и [Nova Privacy Pool](iii.-nova-privacy-pool.md) с гибкими суммами. Оба позволяют вывести средства на другой кошелек без видимой ончейн-связи с исходным депозитом.
...
Solana は高速で取引コストが低い一方、ネイティブなプライバシーはありません。すべての取引は公開され、ユーザーの活動を露出し、機密性を損ないます。Voidify は、固定額の Classic pools と柔軟な金額の [Nova Privacy Pool](iii.-nova-privacy-pool.md) という 2 つのモードでこの欠落を補います。どちらも、元の deposit との明確なオンチェーンリンクを残さず別の wallet へ withdraw できます。
